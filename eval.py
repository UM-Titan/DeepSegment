...
def vis_vid_output(output, image,model, plot_kpts):
    ...
    for idx in range(output.shape[0]):
        # plot_skeleton_kpts(nimg, output[idx, 7:].T, 3)
        kpt_out, kid_out = plot_face_kpts(nimg,output[idx, 7:].T, 3, plot_kpts)
        kpts[idx] = kpt_out
        kids[idx] = kid_out
...
def check_normal(kids,thresh):
    # print (kids)
    # print (thresh)
    # if (0 in kids[0]) and (1 in kids[0]) and (2 in kids[0]) and (4 in kids[0]) and (thresh<4) and not (3 in kids[0]):
    if (0 in kids[0]) and (1 in kids[0]) and (2 in kids[0]) and (4 in kids[0]) and (thresh<4) and (3 in kids[0]):
        return -1
    else:
        return 1
...
def run_vid_(vid_path,plot,vis,rotate,plot_kpts, save_video):
    ...
    while cap.isOpened():
        ...
        if ret:
            ...
            nimg, kpts, kids = vis_vid_output(output, image, model, plot_kpts)
            # print (kpts)
            # print (kids)
            if width_rs is None:
            ...
            if len(kids)>0:
                event_type = check_normal(kids,dist_std_.values[-1])
                print (event_type)
                ...
                # if dist_std_.values[-1] >4 or (3 in kids[0]) or (4 not in kids[0]):
                #     # print (dist_std_.values[-1], fnumber)
                #     event_wrt = 'event'
                
                if threshval > 4 and starts == 0:
                ...
                if len(frame_ids)>0:
                    ...
                    cv2.putText(nimg,event_wrt_1, (100,600), font,
                    fontScale, (255,255,0), thickness,lineType)
                    df.loc[len(df)] = [event_wrt,int(frame_ids[-1]), int(fnumber)]

                cv2.putText(nimg,event_wrt, bottomLeftCornerOfText, font, 
                fontScale, fontColor, thickness,lineType)
            # print (df.tail())
            if df.shape[0]>500:
                df.to_csv('anomalies.csv',index=False,mode='a+',header=False)
                df = pd.DataFrame(columns=['event_type','starts','ends'])
...
## todo
## proximity between 2 and 4 || and 1 and 3
